...
class Users extends Component {
  ...
  markComplete = (id, taskId) => {
    ...
    let newUsersArr = this.state.users;
    let taskIndex = newUsersArr[userIndex].todos.findIndex(
      (item) => item.id === taskId
    );
  ...
  addTask = (id, title) => {
    ...
    // Insert a new Todo
    newUsersArr[userIndex].todos.push({
      userId: id,
      id: lastTaskId + 1,
      title: title,
      completed: false,
  ...
  addPost = (id, title, body) => {
    ...
    // Insert a new Todo
    newUsersArr[userIndex].posts.push({
      userId: id,
      id: lastPostId + 1,
      title: title,
      body: body,
  ...
  getNewUser = (name, email) => {
    ...
    // Getting last UserId
    let nextUserId = newUsersArr[0]
      ? newUsersArr[newUsersArr.length - 1].id + 1
      : 1;
  ...
  render() {
    ...
    if (this.state.searchValue) {
      users = this.state.users.filter(
        (user) =>
          ...
          user.email.includes(this.state.searchValue)
      );
    }
    ...
    if (this.state.isAddUser) {
      addNewUser = (
        <AddUser
          callbackAddUser={(name, email) => this.getNewUser(name, email)}
    ...
    return (
      <div>
        <header>
          <h1>Users Management Web App</h1>
        ...
        <div className="more-info-box">{addNewUser}</div>
        <ul>{users}</ul>
      </div>
    );
  }
