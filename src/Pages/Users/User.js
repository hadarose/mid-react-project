...
import TodosWrap from "../Todos/TodosWrapper";
import PostsWrap from "../Posts/PostsWrapper";
import "../Proj.css";
...
class User extends Component {
  ...
  render() {
    let userClassNames = this.props.user.todos.some((item) => !item.completed)
      ? "redBorderComp"
      : "greenBorderComp";
    let addressComp;
    let todosComp;
    ...
    if (this.state.isOtherData) {
      addressComp = (
        <Address
          ...
          callbackGetAddress={(street, city, zipcode) =>
            this.getAddress(street, city, zipcode)
          }
        />
      );
    }
